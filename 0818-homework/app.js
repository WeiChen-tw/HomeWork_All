...
app.get("/home/img", function (request, response) {

	connection.query('select ImgUrl from news WHERE newsid = 1', 
		'',
		function(err, rows) {
			if (err)	{
				console.log(JSON.stringify(err));
...
app.post("/home/news", function (request, response) {
	...
	connection.query(
		"insert into news set imgUrl = ?, text = ?, title = ?, ymd = ?", 
			[
				request.body.imgUrl,
				request.body.text, 
				request.body.title, 
...
app.put("/home/news", function (request, response) {
	...
	connection.query(
		"update news set imgUrl = ?, text = ?, title = ?, ymd = ? where newsId = " 
		    + request.body.newsId, 
			[
				request.body.imgUrl,
				request.body.text, 
